...
print_modname() {
  ...
  ui_print ""
  ui_print "Gboard Lite Installer Module"
  ui_print ""
  sleep 0.01
  echo "-------------------------------------"
  echo -e "- Module：\c"
  ...
  if [ "$BOOTMODE" ] && [ "$KSU" ]; then
    ui_print "- Root provider: KernelSU App"
    ...
    ui_print "- Root Provider: Magisk App"
    REPLACE="
      /system/product/priv-app/LatinIME
      /system/product/app/LatinIME
      /system/product/app/LatinIMEGooglePrebuilt
...
on_install() {
  ...
  [ -z $MINAPI ] || { [ $API -lt $MINAPI ] && abort "- Your system API, $API, is less than the minimum API of $MINAPI and is not supported."; }
  ...
  VW_APK_URL="https://github.com/driedpampas/gboardlite/raw/main/dist/${ARCH}/base.apk"
  ...
  if [ ! -f "$MODPATH/system/product/app/gboardlite/base.apk" ]; then
    echo "- Error while downloading, please check your connection!"
    exit 1
  fi
